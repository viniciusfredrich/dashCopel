...
function parseBRL(str) {
  ...
  const s = String(str).replace(/\s|R\$\s?/g, '').replace(/\./g, '').replace(',', '.');
  const n = Number(s);
  return isNaN(n) ? 0 : n;
}
...
function formatMiBi(v){
  if (v >= 1e9) return `R$ ${nfBR.format(v/1e9)} bi`;
  if (v >= 1e6) return `R$ ${nfBR.format(v/1e6)} mi`;
  if (v >= 1e3) return `R$ ${nfBR.format(v/1e3)} mil`;
  return `R$ ${nfBR.format(v)}`;
}
...
function formatMiBiShort(v) {
  // versão mais curta para rótulos do donut
  if (v >= 1e9)  return `R$ ${nfBR.format(v/1e9)} bi`;
  if (v >= 1e6)  return `R$ ${nfBR.format(v/1e6)} mi`;
  return `R$ ${nfBR.format(v)}`;
}
...
if (!ativoFiltro) {
  ...
  const x = ['Governo', 'Planejado', 'Pago'];
  const yBi = [totalGov, totalPlan, totalPago].map(v => v / 1e9);
  const hovertext = [totalGov, totalPlan, totalPago].map(formatMiBi); // <<< strings prontas

  traces = [{
    x,
    y: yBi,                       // eixo Y em bilhões
    type: 'bar',
    marker: { color: [CORES[0], CORES[1], CORES[2]] },
    hovertext,                    // <<< usa hovertext
  ...
  traces = [
    {
      ...
      hovertext: [formatMiBi(govValor)],
      hovertemplate: '%{x}<br>%{fullData.name}: %{hovertext}<extra></extra>'
    },
    {
      x: orgaos,
      y: orgaos.map(o => (orgaosPlan[o] || 0) / 1e9),
      name: 'Valor planejado',
      type: 'bar',
      marker: { color: CORES[1] },
      hovertext: orgaos.map(o => formatMiBi(orgaosPlan[o] || 0)),
      hovertemplate: '%{x}<br>%{fullData.name}: %{hovertext}<extra></extra>'
    },
    {
      x: orgaos,
      y: orgaos.map(o => (orgaosPago[o] || 0) / 1e9),
      name: 'Valor pago',
      type: 'bar',
      marker: { color: CORES[2] },
      hovertext: orgaos.map(o => formatMiBi(orgaosPago[o] || 0)),
      hovertemplate: '%{x}<br>%{fullData.name}: %{hovertext}<extra></extra>'
    }
  ];
}
    ...
    const layoutBar = {
      ...
      yaxis: {
        ...
        tickformat: ',.2f',
        tickprefix: 'R$ ',
        ticksuffix: ' bi'   // eixo Y em bilhões
      },
      legend: { orientation: 'h', x: 0.5, xanchor: 'center', y: -0.2 }
    };

    Plotly.newPlot(container, traces, layoutBar, {displayModeBar:false, displaylogo:false})
      .then(() => attachMainClick(Object.keys(gov)));
    ...
    const pie = {
      ...
      values,
      marker: { colors: cores },
      // mostra valor adaptativo + porcentagem
      hovertemplate: '%{label}<br>%{customdata}<br>%{percent}<extra></extra>',
      texttemplate: '%{label}<br>%{customdata} (%{percent})',
      textposition: 'outside',
      customdata: values.map(v => formatMiBiShort(v)),
      domain: { x: [0.08, 0.92], y: [0.10, 0.88] }
    };

    Plotly.newPlot(container, [pie], layoutPie, { displayModeBar:false, displaylogo:false })
      .then(() => attachMainClick(labels));
  }
}
...
function drawExecucaoFisicaChart() {
  ...
  const layout = {
    ...
    xaxis: { automargin: true },
    yaxis: { range: [0, 80], automargin: true }
  };
...
function plotarGraficoHistorico(dados) {
  ...
  dados.forEach(linha => {
    datas.push(toISO(String(linha['DATA'])));

    const saldo = parseBRL(linha['SALDO FINAL']);
    const repasse = parseBRL(linha['TOTAL REPASSE']);

    saldoFinalRaw.push(saldo);
    repassesRaw.push(repasse);

    saldoFinal.push(saldo / 1e9);           // eixo sempre em bi
    repassesAcumulados.push(repasse / 1e9); // idem
  });

  // formatação adaptativa
  const formatarValor = v => {
    if (v >= 1e9) {
      return `R$ ${(v/1e9).toFixed(2)} bi`;
    } else {
      return `R$ ${(v/1e6).toFixed(2)} mi`;
    }
  };
  ...
  const traceSaldo = {
    ...
    customdata: saldoFinalRaw.map(v => formatMiBiShort(v))
  };
  ...
  const traceRepasse = {
    ...
    customdata: repassesRaw.map(v => formatMiBiShort(v))
  };
...
function init() {
  Promise.all([
    ...
    fetch('data/df_novo_csv2409.csv').then(r => r.text()),
